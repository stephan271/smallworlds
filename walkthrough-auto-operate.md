...
We have bolstered the cluster's security posture to ensure the infrastructure remains robust over time:

### 1. Continuous Vulnerability Scanning
- **Trivy Operator**: Deployed `trivy-operator.yaml` into the cluster. This operator continuously scans running container images, Kubernetes manifests, and exposed secrets. Hermes will monitor the `VulnerabilityReport` custom resources generated by Trivy to flag severe CVEs and optionally submit PRs to bump affected image versions.
