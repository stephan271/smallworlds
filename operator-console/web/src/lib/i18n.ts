...
const messages = {
  en: {
    ...
    sourceOperator: 'Operator', sourceGenerated: 'Created by this console',
    ...
    ,capabilityEyebrow: 'Cluster design', capabilityTitle: 'What the infrastructure will offer', capabilityDescription: 'The parts that keep everything running are always installed. Choose the apps your members will use and the address they will visit; the exact configuration this produces is shown for approval on the next stage, before anything is written.', capabilityMode: 'How much to install', capabilityMinimal: 'Minimal', capabilityCollaboration: 'Collaboration', capabilityFull: 'Full', capabilityCustom: 'Custom', capabilityRelease: 'SmallWorlds version', capabilityRepository: 'Your private settings repository', capabilityDomain: 'Your web address', capabilityEnvExtHint: 'Leave empty for the real cluster. Something like .dev gives every address its own label, so a test cluster can never collide with it.', capabilityCommunityApps: 'Community applications', capabilityReview: 'Work out what that needs', capabilityPreview: 'Before anything happens', capabilityPlanReady: 'Deterministic overlay preview', capabilityMemory: 'Estimated memory', capabilityStorage: 'Estimated storage', capabilityOverlayDiff: 'The exact changes', bootstrapAssetEyebrow: 'Bootstrap prerequisites', bootstrapAssetTitle: 'Installer files', bootstrapAssetDescription: 'See exactly which files this version needs and where they come from, before any of them are downloaded. Each one is checked against its signature.', offlineBundleFuture: 'Offline Bundle support is planned future work; initial bootstrap requires internet access.', bootstrapAssetInspect: 'Show which files are needed', bootstrapAssetAcquire: 'Download and check the files', bootstrapAssetUnavailable: 'No compatible signed bootstrap asset manifest is published for this release yet.', nodeEyebrow: 'Cluster node', nodeTitle: 'The computer that will run it', nodeDescription: 'Point this console at the Linux machine that will run your cluster. Nothing is changed — it is only looked at, and only after you confirm you recognise it.', nodeTarget: 'Target', nodeRemote: 'Remote Linux node', nodeSameHost: 'This Linux Launcher Host', nodeHost: 'Its name or address on the network', nodePort: 'SSH port', nodeUsername: 'SSH username', nodeAuthentication: 'SSH authentication', nodeAgent: 'SSH agent', nodePrivateKey: 'Private key', nodePassword: 'Password', nodeKeyPassphrase: 'Private-key passphrase', nodeSudoPassword: 'Sudo password (if required)', nodeProbe: 'Show SSH fingerprint', nodeFingerprint: 'SSH host-key fingerprint', nodeTrust: 'Confirm and trust', nodeInspect: 'Check this computer', nodeOperatingSystem: 'Operating system', nodeCapacity: 'Available capacity', nodeAssessment: 'Assessment', nodeReady: 'Suitable — ready to continue', nodeAlreadyInstalled: 'This computer already carries a finished installation of this community. Whatever an earlier attempt reported, the machine itself says the installation completed — look at what the cluster is doing before installing again.', nodePlanSSHKey: 'Plan dedicated SSH key', nodeFingerprintExpired: 'The fingerprint reading has expired — the launcher only accepts a confirmation for a key it saw in the last ten minutes. Show the fingerprint again and confirm it.', nodeConnectionFailed: 'This computer could not be reached over SSH. Nothing was changed.', nodeCredentialsRejected: 'The SSH login was rejected. Check the username and the key or password.', nodeTargetInvalid: 'The address, port or username is not usable. Check them and try again.', nodeSudoRejected: 'Administrator rights were refused. Enter the sudo password, or allow passwordless sudo for this account.', nodeCleanFailed: 'Removing what is in the way did not finish. Nothing further was attempted.', nodeRecordedHost: 'Address and port', nodeRecordedConfirmedAt: 'Recognised on', nodeRecordedHint: 'This is the computer you already confirmed for this community. Its details are filled in below. Check it again to see its current state before installing anything.', overlayRecordedRepository: 'Settings repository', adoptTitle: 'Put a reviewed change into service', adoptDescription: 'Your cluster runs the exact change you approved, and keeps running it until you say otherwise — merging a proposal does not change what is deployed. When you have merged one where your settings live, name it here.', adoptCurrent: 'Currently running', adoptRevision: 'The merged change', adoptRevisionHint: 'The full identifier of the change, as your Git provider shows it. Not a branch name: a branch can be moved afterwards, and what your cluster runs should not move without you.', adoptAction: 'Put it into service', adoptDone: 'In service. Your cluster confirmed the change and is bringing itself in line with it.', adoptInvalid: 'That is not the full identifier of a change. Copy the long one your Git provider shows for the merge.', adoptUnconfirmed: 'The cluster did not confirm the change, so nothing was recorded. Check that the change really is merged where your settings live, then try again.', adoptFailed: 'The cluster could not be reached to put the change into service. Nothing was changed.', overlayRecordedCommit: 'Last recorded change', overlayRecordedAt: 'Established on', overlayRecordedHint: 'This community already has a settings repository. You do not need a second one — later changes go into this one as a proposal you review.', githubEyebrow: 'Git provider', githubTitle: 'GitHub overlay access', githubDescription: 'Create a fine-grained personal access token. The token is validated and retained only in the encrypted Launcher Vault.', githubTokenGuide: 'Open GitHub token settings', githubAuthority: 'Authority purpose', githubCreationAuthority: 'Temporary repository creation', githubOngoingAuthority: 'Repository-scoped ongoing access', githubToken: 'Fine-grained GitHub token', githubValidate: 'Validate and store token', githubOwner: 'GitHub owner', githubNoExpiry: 'No expiry reported', githubAuthorityUnverified: 'GitHub does not publish what a fine-grained token is allowed to do, so this could not be checked here. If a permission is missing, the step that needs it will say so.', githubRepositoryName: 'Private repository name', githubRepositoryHint: 'This console creates the private repository on GitHub for you and writes the settings into it directly — there is no folder on this computer and nothing for you to set up by hand. If a private repository with this name already exists and is still empty, it is used as it is. Nothing is ever overwritten.', settingsRepoNeedsPlan: 'Choose above where your settings should live. The exact files that will be written appear here as soon as that is settled.', settingsRepoNeedsToken: 'Validate a GitHub token above first. The repository is then created for you.', settingsRepoNeedsCreationAuthority: 'The stored token only has ongoing repository access. To have the repository created for you, validate a token with temporary repository-creation authority.', settingsRepoNeedsAccess: 'Validate the username and token for your empty repository above first.', settingsRepoTokenAuthority: 'This token does not carry the authority this step needs. Check its owner, permissions and repository scope, then validate it again.', settingsRepoRateLimited: 'GitHub is currently rate-limiting this token. Wait a moment and try again — nothing was left half-done.', settingsRepoWriteFailed: 'The repository exists but the settings could not be written into it. Try again: an empty repository is reused rather than duplicated.', settingsRepoPlanStale: 'The file listing is no longer current. Open this stage again to have it rebuilt, then retry.', githubRepositoryNotEmpty: 'A repository with this name already exists and already contains files. It was left untouched — choose a different name, or empty it yourself first.', githubRepositoryNotPrivate: 'A repository with this name already exists but is public. Your settings repository has to be private — choose a different name.', githubEstablish: 'Establish GitHub overlay', genericGitEyebrow: 'Existing Git repository', genericGitTitle: 'HTTPS Git overlay access', genericGitDescription: 'Use an empty HTTPS repository. Its username and token are validated, encrypted in the Launcher Vault, and never returned to the browser.', genericGitUsername: 'Git username', genericGitToken: 'Git access token', genericGitValidate: 'Validate and store access', genericGitApprovalHint: 'Initializing the repository approves the file listing above and writes exactly those files into it.', genericGitEstablish: 'Initialize HTTPS Git overlay', genericGitPropose: 'Push reviewed change branch', genericGitManualMerge: 'Review this branch in your Git provider and merge it manually:', genericGitRemoteNotEmpty: 'That repository already contains files. Nothing was touched — empty it yourself, or point this at an empty repository.', genericGitIdentityConflict: 'This community already deploys from a different repository. Moving it to another one is a separate decision, not part of setting it up.', genericGitAuthenticationFailed: 'The repository rejected the username and token. Check both and validate them again.', genericGitResumeCheckFailed: 'The repository could not be reached to check its current state. Nothing was changed.', localBootstrapEyebrow: 'Cluster installation', localBootstrapTitle: 'Install onto this computer', localBootstrapDescription: 'Reinspect this node and review the privileged, resumable installation before approving it.', localBootstrapEnvironment: 'Environment extension', localBootstrapDataDirectory: "Where your community's data is kept", localBootstrapNodeName: 'Kubernetes node name', localBootstrapACMEEmail: 'Certificate email (optional)', localBootstrapManageDNS: 'Manage public DNS records', localBootstrapSecrets: 'Kubernetes Secret manifests (kept outside Git)', localBootstrapSecretsGenerated: 'This console writes these itself: the Keycloak and Grafana administrator logins, Garage\'s machine credentials, and Argo CD\'s access to your settings repository — that last one from the credential already in the safe. Nothing here is yours to type.', localBootstrapSecretsOverride: 'Optional. Leave this empty. Fill it in only to supply your own values instead — then exactly what you write is used and nothing is created for you.', localBootstrapCredentialsShow: 'Show the administrator logins', localBootstrapCredentialsTitle: 'The logins for your cluster', localBootstrapCredentialsHint: 'Write these down now. They stay in the password safe and travel in your recovery bundle, but nothing else will ever show them to you.', localBootstrapCredentialsKeycloak: 'Keycloak administrator', localBootstrapCredentialsGrafana: 'Grafana administrator', localBootstrapCredentialsNone: 'You supplied these Secrets yourself and they are named differently, so there is nothing here to show.', localBootstrapReview: 'Reinspect and create Change Plan', localBootstrapOverlayCommit: 'GitOps overlay commit', localBootstrapOverlayRequired: 'This community has no settings repository yet. Set one up in the previous stage — the installation is deployed from it, so there is nothing to install without it.', localBootstrapReleaseMismatch: 'This installation would use a different version than the {overlay} your settings repository is pinned to. Change the version in the cluster design stage back to {overlay}, or establish the repository again at the version you want.', localBootstrapReleaseUnsupported: 'That is not a valid SmallWorlds version identifier.', localBootstrapDomainMismatch: 'The web address here is not the one recorded in your settings repository. Make the two match before installing.', localBootstrapAssetsNotReady: 'The installer files have not been downloaded and checked yet. This stage fetches them by itself; wait for it, or try again if it reported a failure.', localBootstrapHostKeyRequired: "Confirm this computer's SSH fingerprint above before installing onto it.", localBootstrapHostKeyMismatch: 'This computer now presents a different SSH host key than the one you confirmed. Nothing was changed — find out why before continuing.', localBootstrapReinspectionFailed: 'This computer could not be checked again just now. Nothing was changed.', localBootstrapPreconditionsFailed: 'This computer is not ready for the installation. Nothing was changed. What is in the way:', localBootstrapInvalidSecrets: 'The Kubernetes Secret manifests are not valid YAML.', localBootstrapDNSTokenRequired: 'Managing public DNS records needs a Hetzner DNS token. Add one or turn that option off.', localBootstrapRouterRequired: 'Confirm the router port forwards before an internet-exposed installation can be planned.', handoffEyebrow: 'Handing over administration', handoffTitle: 'Hand administration to the cluster', handoffDescription: 'Set up trusted HTTPS and private-only access, connect this computer, then hand day-to-day administration to the first owner. Nothing is put on the internet.', handoffUnlockFirst: 'Open the password safe first — these steps need what is in it.', handoffStepsTitle: 'Progress', handoffStepClusterCA: "This device trusts your cluster's certificates", handoffStepPrivateNetwork: 'Private network and its addresses set up', handoffStepLauncherEnrolled: 'This computer joined the private network', handoffStepGatewayIdentity: 'The private entrance has its own identity', handoffStepGatewayAccess: 'Only encrypted connections are accepted', handoffStepVerified: 'Checked: reachable privately, addresses resolve, certificates valid', handoffStepClosed: 'Temporary direct access removed', handoffStepFirstOwner: 'First owner registered', handoffClusterCAEstablish: "Create your cluster's certificate authority", handoffDeviceTrustInstall: 'Trust it on this device', handoffDeviceTrustFingerprint: 'Certificate fingerprint', handoffBaseDomain: 'Private web address', handoffPrivateNetworkEstablish: 'Set up the private network', handoffTailscaleDetect: 'Look for the Tailscale app', handoffTailscaleDetected: 'The official Tailscale app is installed on this computer.', handoffTailscaleAbsent: 'No Tailscale app found on this computer.', handoffTailscaleAcquire: 'A checked download is available. Installing it needs administrator rights.', handoffTailscaleManual: 'Install the official app yourself', handoffEnrollmentEstablish: 'Create the credentials for joining', handoffLauncherConsume: 'Use the one-time credential to join', handoffVerify: 'Check that private access works', handoffCloseAccess: 'Remove temporary direct access', handoffFirstOwnerClaim: 'Start registering the first owner', handoffFirstOwnerRegister: 'Register a passkey and close the setup door for good', handoffLimitations: 'What private-only means', handoffConsoleUrl: "Your cluster's own console"
    ...
    // --- Standing sections -------------------------------------------------
    // Setting an installation up is one thing; looking after the installation
    // you already have is another. They are separate places rather than one
    // long page, so a step never has to be read past to reach either.
    ,clusterDetailEyebrow: 'Read from the cluster itself'
    ,clusterDetailTitle: 'What the cluster is doing'
    ,clusterDetailRefresh: 'Read it again'
    ,clusterDetailReading: 'Reading the cluster…'
    ,clusterDetailObservedAt: 'Observed'
    ,clusterDetailNodes: 'The machine'
    ,clusterDetailNodeReady: 'ready'
    ,clusterDetailNodeNotReady: 'not ready'
    ,clusterDetailNoNodes: 'No machine has reported in yet.'
    ,clusterDetailApplications: 'What Argo CD is installing'
    ,clusterDetailNoApplications: 'Argo CD has not registered any applications yet.'
    ,clusterDetailApplication: 'Application'
    ,clusterDetailSync: 'Configuration delivered'
    ,clusterDetailHealth: 'Running'
    ,clusterDetailWorkloads: 'What is not running yet'
    ,clusterDetailWorkloadsSettled: 'Everything that should be running is running.'
    ,clusterDetailWorkloadsHint: 'Only what has not settled. Anything running normally is left out, so what is in the way is not buried.'
    ,clusterDetailWorkload: 'Workload'
    ,clusterDetailState: 'State'
    ,clusterDetailReason: 'What the cluster says'
    ,clusterDetailRestarts: 'Restarts'
    ,clusterDetailTruncated: 'More are in the same state than are listed here.'
    ,clusterDetailMarkers: 'Installation milestones recorded on the machine'
    ,clusterDetailNotInstalled: 'Nothing has been installed for this community yet, so there is no cluster to read.'
    ,clusterDetailUnreachable: 'The machine could not be reached just now. Nothing was changed; try reading it again.'
    ,clusterDetailUnavailable: 'The cluster could not be read just now.'
    ,activityRecordHeading: 'What this console did'
    ,localBootstrapSecretsHint: 'Required. The passwords your cluster needs for Keycloak, Garage, Grafana and its own settings repository. They go straight into the cluster and never into Git. Paste one or more Kubernetes Secret manifests, separated by ---.'
    ,localBootstrapSecretsRequired: 'Without these the installation finishes and then never starts: Keycloak, Garage and Grafana each wait for a password that only you can supply, and nothing else can tell you that is why.'
    ,vaultEyebrow: 'Unlock to continue'
    ,sections: 'Sections'
    ,tabSetup: 'Setting up'
    ,tabActivity: 'What happened'
    ,tabManage: 'This installation'
    ,tabSetupSummary: 'The decisions still to make, in the order they can be made.'
    ,tabActivitySummary: 'Everything this console has recorded for this installation, newest last.'
    ,tabManageSummary: 'Details, stored access, recovery, and shutting the installation down.'

    ,nextAction: 'Do this next'
    ,nextActionGo: 'Take me there'
    ,nextActionDone: 'Nothing is waiting for you. Every stage of setting this installation up is finished.'
    ,stepBack: 'Back'
    ,stepBlockedInstallFirst: 'First install the cluster.'

    ,stepInstallTitle: 'Install it'
    ,stepInstallSummary: 'Read exactly what will be done, approve it, and watch it happen.'
    ,stepFinishTitle: 'Finish and hand over'
    ,stepFinishSummary: 'A last honest look at what is healthy and what is not, then administration moves into the cluster.'

    // Fetched rather than asked for: the version was already chosen, and there
    // is no second decision hiding in "which files does that need".
    ,installerFilesReady: 'Installer files for {release} downloaded and checked.'
    ,installerFilesWorking: 'Downloading and checking the installer files for {release}…'
    ,installerFilesFailed: 'The installer files could not be prepared.'
    ,installerFilesRetry: 'Try again'
    ,installerFilesWhat: 'What was fetched'

    ,identityHeading: 'Naming and contact'
    ,identityEmail: 'Administration email address'
    ,identityEmailHint: "Certificate expiry warnings and administrative notices go here. It is an ordinary contact detail, not a credential."
    ,designSummaryHeading: 'What that adds up to'
    ,designApps: 'Community applications'
    ,designAppsNone: 'None — a valid installation, just without member-facing apps.'
    ,designPlatform: 'Always installed'
    ,designPlatformHint: 'Identity, databases, storage, certificates, monitoring, backup, mail delivery and private networking are what the applications stand on. They cannot be turned off.'
    ,designExposure: 'Reachable by'
    ,designProtection: 'Data that must be backed up'

    ,advancedShow: 'Advanced settings'
    ,advancedHide: 'Hide advanced settings'
    ,advancedHint: 'Sensible values are already filled in. Open this only if you need something different.'

    ,settingsRepoFilesHeading: 'The exact files that will be written'
    ,settingsRepoFilesHint: 'This is the whole configuration of your cloud, in readable form. No password, token or key appears in it.'
    ,settingsRepoPreparing: 'Working out exactly what will be written…'

    ,installReadyHint: 'Nothing has been changed yet. Approving the plan below is the first step that does.'
    ,installWatching: 'Argo CD is now reading your settings repository and installing what you chose, in dependency order. Certificates in particular can take a while.'
    ,installDone: 'The cluster reported the expected components as ready.'

    ,handoffNextHeading: 'Next step'
    ,handoffActionCA: "Create your cluster's certificate authority and trust it on this device"
    ,handoffActionEnroll: 'Join this computer to the private network'
    ,handoffActionOwner: 'Register your passkey as the first owner'
    ,handoffComplete: 'Administration is private. The temporary ways in are closed.'

    ,finishOutstanding: 'Not finished yet'
    ,finishHealthy: 'Everything this console can check is in order.'
    ,finishConsoleHint: 'From here on you administer this installation at its own private address, through the private network. The launcher has done its job and can be closed.'
    ,finishProtectionMissing: 'No offsite copy has been proved to arrive yet. Until it has, one hardware failure is the end of this community.'

    ,activityEmpty: 'Nothing has been recorded for this installation yet.'
    ,activityRunStarted: 'Work started'
    ,activityRunCheckpoint: 'Progress recorded'
    ,activityRunFailed: 'Work failed'
    ,activityOffsiteProposed: 'Offsite backup change proposed'
    ,activityDecommissionApproved: 'Shutdown approved'
    ,activityDecommissionCompleted: 'Shutdown completed'
    ,activityRedactionNote: 'Secrets never appear here, in plans, or in exports.'

    ,manageDetails: 'Installation details'
    ,manageCredentials: 'What the safe holds'
    ,manageDiagnostics: 'Diagnostics'
    ,retireNotYet: 'There is nothing to shut down: this installation has not been built yet. Removing it from this computer is enough.'
  ...
  de: {
    ...
    rotationStatus: 'Rotation',
    sourceOperator: 'Operator', sourceGenerated: 'Von dieser Konsole erzeugt',
    ...
    ,capabilityEyebrow: 'Cluster-Entwurf', capabilityTitle: 'Was die Infrastruktur anbieten wird', capabilityDescription: 'Die Bestandteile, die alles am Laufen halten, werden immer installiert. Wählen Sie die Anwendungen für Ihre Mitglieder und die Adresse, unter der sie erreichbar sind; die daraus entstehende Konfiguration wird in der nächsten Stufe zur Freigabe angezeigt, bevor irgendetwas geschrieben wird.', capabilityMode: 'Wie viel installiert wird', capabilityMinimal: 'Minimal', capabilityCollaboration: 'Zusammenarbeit', capabilityFull: 'Vollständig', capabilityCustom: 'Benutzerdefiniert', capabilityRelease: 'SmallWorlds-Version', capabilityRepository: 'Ihr privates Einstellungs-Repository', capabilityDomain: 'Ihre Webadresse', capabilityEnvExtHint: 'Für den echten Cluster leer lassen. Etwas wie .dev gibt jeder Adresse eine eigene Kennung, damit ein Testcluster nie mit ihm kollidieren kann.', capabilityCommunityApps: 'Community-Anwendungen', capabilityReview: 'Ermitteln, was das braucht', capabilityPreview: 'Bevor etwas passiert', capabilityPlanReady: 'Deterministische Overlay-Vorschau', capabilityMemory: 'Geschätzter Arbeitsspeicher', capabilityStorage: 'Geschätzter Speicher', capabilityOverlayDiff: 'Die genauen Änderungen', bootstrapAssetEyebrow: 'Bootstrap-Voraussetzungen', bootstrapAssetTitle: 'Installationsdateien', bootstrapAssetDescription: 'Sehen Sie genau, welche Dateien diese Version benötigt und woher sie kommen, bevor eine davon geladen wird. Jede wird gegen ihre Signatur geprüft.', offlineBundleFuture: 'Offline-Bundle-Unterstützung ist geplante Zukunftsarbeit; der erste Bootstrap benötigt Internetzugang.', bootstrapAssetInspect: 'Benötigte Dateien anzeigen', bootstrapAssetAcquire: 'Dateien herunterladen und prüfen', bootstrapAssetUnavailable: 'Für diese Version ist noch kein kompatibles signiertes Bootstrap-Asset-Manifest veröffentlicht.', nodeEyebrow: 'Cluster-Knoten', nodeTitle: 'Der Computer, der alles betreibt', nodeDescription: 'Richten Sie diese Konsole auf den Linux-Rechner, der Ihren Cluster betreiben soll. Es wird nichts verändert — er wird nur angesehen, und erst nachdem Sie ihn wiedererkannt haben.', nodeTarget: 'Ziel', nodeRemote: 'Entfernter Linux-Knoten', nodeSameHost: 'Dieser Linux-Launcher-Host', nodeHost: 'Sein Name oder seine Adresse im Netz', nodePort: 'SSH-Port', nodeUsername: 'SSH-Benutzername', nodeAuthentication: 'SSH-Anmeldung', nodeAgent: 'SSH-Agent', nodePrivateKey: 'Privater Schlüssel', nodePassword: 'Passwort', nodeKeyPassphrase: 'Passphrase für privaten Schlüssel', nodeSudoPassword: 'Sudo-Passwort (falls erforderlich)', nodeProbe: 'SSH-Fingerabdruck anzeigen', nodeFingerprint: 'SSH-Hostschlüssel-Fingerabdruck', nodeTrust: 'Bestätigen und vertrauen', nodeInspect: 'Diesen Computer prüfen', nodeOperatingSystem: 'Betriebssystem', nodeCapacity: 'Verfügbare Kapazität', nodeAssessment: 'Bewertung', nodeReady: 'Geeignet — bereit zum Weitermachen', nodeAlreadyInstalled: 'Auf diesem Computer liegt bereits eine abgeschlossene Installation dieser Gemeinschaft. Was ein früherer Versuch gemeldet hat, ändert daran nichts — der Rechner selbst sagt, dass die Installation fertig geworden ist. Sehen Sie nach, was der Cluster tut, bevor Sie erneut installieren.', nodeFingerprintExpired: 'Die Fingerabdruck-Anzeige ist abgelaufen — der Launcher nimmt eine Bestätigung nur für einen Schlüssel an, den er in den letzten zehn Minuten gesehen hat. Lassen Sie den Fingerabdruck erneut anzeigen und bestätigen Sie ihn.', nodeConnectionFailed: 'Dieser Computer war über SSH nicht erreichbar. Es wurde nichts verändert.', nodeCredentialsRejected: 'Die SSH-Anmeldung wurde abgelehnt. Prüfen Sie Benutzername und Schlüssel bzw. Passwort.', nodeTargetInvalid: 'Adresse, Port oder Benutzername sind nicht verwendbar. Prüfen Sie die Angaben und versuchen Sie es erneut.', nodeSudoRejected: 'Administratorrechte wurden verweigert. Geben Sie das Sudo-Passwort ein oder erlauben Sie passwortloses sudo für dieses Konto.', nodeCleanFailed: 'Das Entfernen wurde nicht abgeschlossen. Es wurde nichts weiter versucht.', nodeRecordedHost: 'Adresse und Port', nodeRecordedConfirmedAt: 'Wiedererkannt am', nodeRecordedHint: 'Das ist der Computer, den Sie für diese Gemeinschaft bereits bestätigt haben. Seine Angaben sind unten eingetragen. Prüfen Sie ihn erneut, um seinen aktuellen Zustand zu sehen, bevor etwas installiert wird.', overlayRecordedRepository: 'Einstellungs-Repository', adoptTitle: 'Eine geprüfte Änderung in Betrieb nehmen', adoptDescription: 'Ihr Cluster fährt genau die Änderung, die Sie freigegeben haben, und bleibt dabei, bis Sie etwas anderes sagen — ein zusammengeführter Vorschlag ändert daran noch nichts. Wenn Sie dort, wo Ihre Einstellungen liegen, eine Änderung zusammengeführt haben, benennen Sie sie hier.', adoptCurrent: 'Läuft derzeit', adoptRevision: 'Die zusammengeführte Änderung', adoptRevisionHint: 'Die vollständige Kennung der Änderung, wie Ihr Git-Anbieter sie anzeigt. Kein Zweigname: ein Zweig lässt sich nachträglich verschieben, und was Ihr Cluster fährt, sollte sich nicht ohne Sie verschieben.', adoptAction: 'In Betrieb nehmen', adoptDone: 'In Betrieb. Der Cluster hat die Änderung bestätigt und gleicht sich daran an.', adoptInvalid: 'Das ist nicht die vollständige Kennung einer Änderung. Kopieren Sie die lange Kennung, die Ihr Git-Anbieter für die Zusammenführung anzeigt.', adoptUnconfirmed: 'Der Cluster hat die Änderung nicht bestätigt, es wurde nichts aufgezeichnet. Prüfen Sie, ob sie dort, wo Ihre Einstellungen liegen, wirklich zusammengeführt ist, und versuchen Sie es erneut.', adoptFailed: 'Der Cluster war nicht erreichbar, um die Änderung in Betrieb zu nehmen. Es wurde nichts verändert.', overlayRecordedCommit: 'Letzte festgehaltene Änderung', overlayRecordedAt: 'Eingerichtet am', overlayRecordedHint: 'Diese Gemeinschaft hat bereits ein Einstellungs-Repository. Sie brauchen kein zweites — spätere Änderungen gehen als Vorschlag zur Prüfung in dieses hier.', githubEyebrow: 'Git-Anbieter', githubTitle: 'GitHub-Overlay-Zugriff', githubDescription: 'Erstellen Sie ein feingranulares persönliches Zugriffstoken. Es wird nur im verschlüsselten Launcher-Tresor validiert und aufbewahrt.', githubTokenGuide: 'GitHub-Token-Einstellungen öffnen', githubAuthority: 'Berechtigungszweck', githubCreationAuthority: 'Temporäre Repository-Erstellung', githubOngoingAuthority: 'Repository-beschränkter Dauerzugriff', githubToken: 'Feingranulares GitHub-Token', githubValidate: 'Token prüfen und speichern', githubOwner: 'GitHub-Inhaber', githubNoExpiry: 'Kein Ablaufdatum gemeldet', githubAuthorityUnverified: 'GitHub veröffentlicht nicht, was ein feingranulares Token darf; das ließ sich hier also nicht prüfen. Fehlt eine Berechtigung, meldet es der Schritt, der sie braucht.', githubRepositoryName: 'Name des privaten Repositorys', githubRepositoryHint: 'Diese Konsole legt das private Repository auf GitHub selbst an und schreibt die Einstellungen direkt hinein — es gibt keinen Ordner auf diesem Computer und nichts, was Sie von Hand einrichten müssten. Existiert bereits ein privates Repository mit diesem Namen und ist es noch leer, wird es unverändert verwendet. Es wird nie etwas überschrieben.', settingsRepoNeedsPlan: 'Wählen Sie oben, wo Ihre Einstellungen liegen sollen. Sobald das feststeht, erscheinen hier die Dateien, die genau so geschrieben werden.', settingsRepoNeedsToken: 'Prüfen Sie zuerst oben ein GitHub-Token. Das Repository wird dann für Sie angelegt.', settingsRepoNeedsCreationAuthority: 'Das gespeicherte Token hat nur dauerhaften Repository-Zugriff. Damit das Repository für Sie angelegt wird, prüfen Sie ein Token mit temporärer Berechtigung zur Repository-Erstellung.', settingsRepoNeedsAccess: 'Prüfen Sie zuerst oben Benutzername und Token für Ihr leeres Repository.', settingsRepoTokenAuthority: 'Dieses Token hat nicht die Berechtigung, die dieser Schritt braucht. Prüfen Sie Inhaber, Rechte und Repository-Geltungsbereich und lassen Sie es erneut prüfen.', settingsRepoRateLimited: 'GitHub begrenzt derzeit die Anfragen dieses Tokens. Warten Sie einen Moment und versuchen Sie es erneut — es ist nichts halb erledigt geblieben.', settingsRepoWriteFailed: 'Das Repository existiert, aber die Einstellungen konnten nicht hineingeschrieben werden. Versuchen Sie es erneut: ein leeres Repository wird weiterverwendet und nicht doppelt angelegt.', settingsRepoPlanStale: 'Die Dateiliste ist nicht mehr aktuell. Öffnen Sie diese Stufe erneut, damit sie neu erstellt wird, und versuchen Sie es dann noch einmal.', githubRepositoryNotEmpty: 'Ein Repository mit diesem Namen existiert bereits und enthält schon Dateien. Es wurde nicht angetastet — wählen Sie einen anderen Namen oder leeren Sie es zuerst selbst.', githubRepositoryNotPrivate: 'Ein Repository mit diesem Namen existiert bereits, ist aber öffentlich. Ihr Einstellungs-Repository muss privat sein — wählen Sie einen anderen Namen.', githubEstablish: 'GitHub-Overlay einrichten', genericGitEyebrow: 'Vorhandenes Git-Repository', genericGitTitle: 'HTTPS-Git-Overlay-Zugriff', genericGitDescription: 'Verwenden Sie ein leeres HTTPS-Repository. Benutzername und Token werden geprüft, im Launcher-Tresor verschlüsselt und nie an den Browser zurückgegeben.', genericGitUsername: 'Git-Benutzername', genericGitToken: 'Git-Zugriffstoken', genericGitValidate: 'Zugriff prüfen und speichern', genericGitApprovalHint: 'Das Initialisieren gibt die Dateiliste oben frei und schreibt genau diese Dateien in das Repository.', genericGitEstablish: 'HTTPS-Git-Overlay initialisieren', genericGitPropose: 'Geprüften Änderungszweig übertragen', genericGitManualMerge: 'Prüfen Sie diesen Zweig bei Ihrem Git-Anbieter und führen Sie ihn manuell zusammen:', genericGitRemoteNotEmpty: 'Dieses Repository enthält bereits Dateien. Es wurde nichts angetastet — leeren Sie es selbst oder verweisen Sie auf ein leeres Repository.', genericGitIdentityConflict: 'Diese Gemeinschaft wird bereits aus einem anderen Repository ausgerollt. Der Umzug auf ein anderes ist eine eigene Entscheidung und nicht Teil der Einrichtung.', genericGitAuthenticationFailed: 'Das Repository hat Benutzername und Token abgelehnt. Prüfen Sie beides und lassen Sie es erneut prüfen.', genericGitResumeCheckFailed: 'Das Repository war nicht erreichbar, um seinen aktuellen Zustand zu prüfen. Es wurde nichts verändert.', localBootstrapEyebrow: 'Cluster-Installation', localBootstrapTitle: 'Auf diesem Computer installieren', localBootstrapDescription: 'Prüfen Sie diesen Knoten erneut und kontrollieren Sie die privilegierte, fortsetzbare Installation vor der Genehmigung.', localBootstrapEnvironment: 'Umgebungserweiterung', localBootstrapDataDirectory: 'Wo die Daten Ihrer Gemeinschaft liegen', localBootstrapNodeName: 'Kubernetes-Knotenname', localBootstrapACMEEmail: 'E-Mail für Zertifikate (optional)', localBootstrapManageDNS: 'Öffentliche DNS-Einträge verwalten', localBootstrapSecrets: 'Kubernetes-Secret-Manifeste (außerhalb von Git)', localBootstrapSecretsGenerated: 'Diese Konsole schreibt sie selbst: die Administrator-Zugänge für Keycloak und Grafana, die Maschinen-Zugangsdaten für Garage und den Zugriff von Argo CD auf Ihr Einstellungs-Repository — letzteren aus dem Zugangsdatum, das ohnehin im Tresor liegt. Hier ist nichts von Ihnen einzutragen.', localBootstrapSecretsOverride: 'Optional. Lassen Sie das leer. Füllen Sie es nur aus, wenn Sie stattdessen eigene Werte vorgeben wollen — dann wird genau das verwendet, was Sie schreiben, und nichts für Sie erzeugt.', localBootstrapCredentialsShow: 'Die Administrator-Zugänge anzeigen', localBootstrapCredentialsTitle: 'Die Zugänge zu Ihrem Cluster', localBootstrapCredentialsHint: 'Schreiben Sie sie jetzt auf. Sie bleiben im Passwort-Tresor und liegen in Ihrem Wiederherstellungspaket, aber sonst zeigt sie Ihnen nichts mehr.', localBootstrapCredentialsKeycloak: 'Keycloak-Administrator', localBootstrapCredentialsGrafana: 'Grafana-Administrator', localBootstrapCredentialsNone: 'Diese Secrets haben Sie selbst vorgegeben und anders benannt; hier gibt es daher nichts anzuzeigen.', localBootstrapReview: 'Erneut prüfen und Änderungsplan erstellen', localBootstrapOverlayCommit: 'GitOps-Overlay-Commit', localBootstrapOverlayRequired: 'Diese Gemeinschaft hat noch kein Einstellungs-Repository. Richten Sie es in der vorherigen Stufe ein — die Installation wird daraus ausgerollt, ohne es gibt es nichts zu installieren.', localBootstrapReleaseMismatch: 'Diese Installation würde eine andere Version verwenden als die {overlay}, auf die Ihr Einstellungs-Repository festgelegt ist. Setzen Sie die Version in der Stufe „Cluster-Entwurf“ auf {overlay} zurück, oder richten Sie das Repository erneut mit der gewünschten Version ein.', localBootstrapReleaseUnsupported: 'Das ist keine gültige SmallWorlds-Versionsbezeichnung.', localBootstrapDomainMismatch: 'Die hier angegebene Webadresse ist nicht die, die in Ihrem Einstellungs-Repository festgehalten ist. Bringen Sie beide in Übereinstimmung, bevor Sie installieren.', localBootstrapAssetsNotReady: 'Die Installationsdateien wurden noch nicht heruntergeladen und geprüft. Diese Stufe holt sie von selbst; warten Sie ab oder versuchen Sie es erneut, wenn ein Fehler gemeldet wurde.', localBootstrapHostKeyRequired: 'Bestätigen Sie oben den SSH-Fingerabdruck dieses Computers, bevor darauf installiert wird.', localBootstrapHostKeyMismatch: 'Dieser Computer zeigt jetzt einen anderen SSH-Hostschlüssel als den, den Sie bestätigt haben. Es wurde nichts verändert — klären Sie die Ursache, bevor Sie weitermachen.', localBootstrapReinspectionFailed: 'Dieser Computer konnte gerade nicht erneut geprüft werden. Es wurde nichts verändert.', localBootstrapPreconditionsFailed: 'Dieser Computer ist noch nicht bereit für die Installation. Es wurde nichts verändert. Was im Weg steht:', localBootstrapInvalidSecrets: 'Die Kubernetes-Secret-Manifeste sind kein gültiges YAML.', localBootstrapDNSTokenRequired: 'Das Verwalten öffentlicher DNS-Einträge benötigt ein Hetzner-DNS-Token. Fügen Sie eines hinzu oder schalten Sie die Option ab.', localBootstrapRouterRequired: 'Bestätigen Sie die Portweiterleitungen am Router, bevor eine im Internet erreichbare Installation geplant werden kann.', handoffEyebrow: 'Verwaltung übergeben', handoffTitle: 'Verwaltung an den Cluster übergeben', handoffDescription: 'Richten Sie vertrauenswürdiges HTTPS und rein privaten Zugang ein, verbinden Sie diesen Computer und übergeben Sie dann die tägliche Verwaltung an die erste Eigentümerin oder den ersten Eigentümer. Nichts davon gelangt ins Internet.', handoffUnlockFirst: 'Öffnen Sie zuerst den Passwort-Tresor — diese Schritte brauchen, was darin liegt.', handoffStepsTitle: 'Fortschritt', handoffStepClusterCA: 'Dieses Gerät vertraut den Zertifikaten Ihres Clusters', handoffStepPrivateNetwork: 'Privates Netzwerk und seine Adressen eingerichtet', handoffStepLauncherEnrolled: 'Dieser Computer ist dem privaten Netzwerk beigetreten', handoffStepGatewayIdentity: 'Der private Zugang hat eine eigene Identität', handoffStepGatewayAccess: 'Es werden nur verschlüsselte Verbindungen angenommen', handoffStepVerified: 'Geprüft: privat erreichbar, Adressen lösen auf, Zertifikate gültig', handoffStepClosed: 'Vorübergehender Direktzugang entfernt', handoffStepFirstOwner: 'Erste Eigentümerin oder erster Eigentümer registriert', handoffClusterCAEstablish: 'Zertifizierungsstelle Ihres Clusters anlegen', handoffDeviceTrustInstall: 'Auf diesem Gerät vertrauen', handoffDeviceTrustFingerprint: 'Zertifikats-Fingerabdruck', handoffBaseDomain: 'Private Webadresse', handoffPrivateNetworkEstablish: 'Privates Netzwerk einrichten', handoffTailscaleDetect: 'Nach der Tailscale-App suchen', handoffTailscaleDetected: 'Die offizielle Tailscale-App ist auf diesem Computer installiert.', handoffTailscaleAbsent: 'Auf diesem Computer wurde keine Tailscale-App gefunden.', handoffTailscaleAcquire: 'Ein geprüfter Download ist verfügbar. Die Installation erfordert Administratorrechte.', handoffTailscaleManual: 'Die offizielle App selbst installieren', handoffEnrollmentEstablish: 'Zugangsdaten für den Beitritt erstellen', handoffLauncherConsume: 'Mit dem Einmal-Zugangsdatum beitreten', handoffVerify: 'Prüfen, ob der private Zugang funktioniert', handoffCloseAccess: 'Vorübergehenden Direktzugang entfernen', handoffFirstOwnerClaim: 'Registrierung der ersten Eigentümerin beginnen', handoffFirstOwnerRegister: 'Passkey registrieren und die Einrichtungstür endgültig schließen', handoffLimitations: 'Was „nur privat“ bedeutet', handoffConsoleUrl: 'Die clustereigene Konsole'
    ...
    ,clusterDetailEyebrow: 'Direkt aus dem Cluster gelesen'
    ,clusterDetailTitle: 'Was der Cluster gerade tut'
    ,clusterDetailRefresh: 'Erneut lesen'
    ,clusterDetailReading: 'Cluster wird gelesen …'
    ,clusterDetailObservedAt: 'Beobachtet'
    ,clusterDetailNodes: 'Der Rechner'
    ,clusterDetailNodeReady: 'bereit'
    ,clusterDetailNodeNotReady: 'nicht bereit'
    ,clusterDetailNoNodes: 'Es hat sich noch kein Rechner gemeldet.'
    ,clusterDetailApplications: 'Was Argo CD installiert'
    ,clusterDetailNoApplications: 'Argo CD kennt noch keine Anwendungen.'
    ,clusterDetailApplication: 'Anwendung'
    ,clusterDetailSync: 'Konfiguration ausgeliefert'
    ,clusterDetailHealth: 'Läuft'
    ,clusterDetailWorkloads: 'Was noch nicht läuft'
    ,clusterDetailWorkloadsSettled: 'Alles, was laufen soll, läuft.'
    ,clusterDetailWorkloadsHint: 'Nur was noch nicht in Ordnung ist. Was normal läuft, bleibt weg, damit das Hinderliche nicht untergeht.'
    ,clusterDetailWorkload: 'Arbeitslast'
    ,clusterDetailState: 'Zustand'
    ,clusterDetailReason: 'Was der Cluster sagt'
    ,clusterDetailRestarts: 'Neustarts'
    ,clusterDetailTruncated: 'Es befinden sich mehr im selben Zustand, als hier aufgeführt sind.'
    ,clusterDetailMarkers: 'Auf dem Rechner festgehaltene Meilensteine der Installation'
    ,clusterDetailNotInstalled: 'Für diese Gemeinschaft wurde noch nichts installiert, es gibt also keinen Cluster zu lesen.'
    ,clusterDetailUnreachable: 'Der Rechner war gerade nicht erreichbar. Es wurde nichts verändert; versuchen Sie es erneut.'
    ,clusterDetailUnavailable: 'Der Cluster konnte gerade nicht gelesen werden.'
    ,activityRecordHeading: 'Was diese Konsole getan hat'
    ,localBootstrapSecretsHint: 'Erforderlich. Die Passwörter, die Ihr Cluster für Keycloak, Garage, Grafana und sein eigenes Einstellungs-Repository braucht. Sie gehen direkt in den Cluster und nie nach Git. Fügen Sie ein oder mehrere Kubernetes-Secret-Manifeste ein, getrennt durch ---.'
    ,localBootstrapSecretsRequired: 'Ohne sie wird die Installation fertig und startet dann nie: Keycloak, Garage und Grafana warten jeweils auf ein Passwort, das nur Sie liefern können, und nichts sonst kann Ihnen sagen, dass es daran liegt.'
    ,vaultEyebrow: 'Zum Fortfahren entsperren'
    ,sections: 'Bereiche'
    ,tabSetup: 'Einrichtung'
    ,tabActivity: 'Was passiert ist'
    ,tabManage: 'Diese Installation'
    ,tabSetupSummary: 'Die noch offenen Entscheidungen, in der Reihenfolge, in der sie möglich sind.'
    ,tabActivitySummary: 'Alles, was diese Konsole für diese Installation festgehalten hat, das Neueste zuletzt.'
    ,tabManageSummary: 'Angaben, hinterlegte Zugänge, Wiederherstellung und Stilllegung.'

    ,nextAction: 'Als Nächstes'
    ,nextActionGo: 'Dorthin wechseln'
    ,nextActionDone: 'Es wartet nichts auf Sie. Die Einrichtung dieser Installation ist abgeschlossen.'
    ,stepBack: 'Zurück'
    ,stepBlockedInstallFirst: 'Installieren Sie zuerst den Cluster.'

    ,stepInstallTitle: 'Installieren'
    ,stepInstallSummary: 'Genau lesen, was getan wird, es genehmigen und dabei zusehen.'
    ,stepFinishTitle: 'Abschließen und übergeben'
    ,stepFinishSummary: 'Ein letzter ehrlicher Blick darauf, was in Ordnung ist und was nicht — danach wandert die Verwaltung in den Cluster.'

    ,installerFilesReady: 'Installationsdateien für {release} geladen und geprüft.'
    ,installerFilesWorking: 'Installationsdateien für {release} werden geladen und geprüft …'
    ,installerFilesFailed: 'Die Installationsdateien konnten nicht bereitgestellt werden.'
    ,installerFilesRetry: 'Erneut versuchen'
    ,installerFilesWhat: 'Was geladen wurde'

    ,identityHeading: 'Benennung und Kontakt'
    ,identityEmail: 'E-Mail-Adresse für die Verwaltung'
    ,identityEmailHint: 'Hierher gehen Hinweise auf ablaufende Zertifikate und Verwaltungsmitteilungen. Das ist eine gewöhnliche Kontaktangabe, kein Zugangsschlüssel.'
    ,designSummaryHeading: 'Was daraus folgt'
    ,designApps: 'Anwendungen für die Gemeinschaft'
    ,designAppsNone: 'Keine — auch das ist eine gültige Installation, nur ohne Anwendungen für die Mitglieder.'
    ,designPlatform: 'Immer installiert'
    ,designPlatformHint: 'Identität, Datenbanken, Speicher, Zertifikate, Überwachung, Sicherungen, Mailversand und das private Netz sind das Fundament der Anwendungen. Sie lassen sich nicht abwählen.'
    ,designExposure: 'Erreichbar für'
    ,designProtection: 'Daten, die gesichert werden müssen'

    ,advancedShow: 'Erweiterte Einstellungen'
    ,advancedHide: 'Erweiterte Einstellungen ausblenden'
    ,advancedHint: 'Sinnvolle Werte sind bereits eingetragen. Öffnen Sie das nur, wenn Sie etwas anderes brauchen.'

    ,settingsRepoFilesHeading: 'Die Dateien, die genau so geschrieben werden'
    ,settingsRepoFilesHint: 'Das ist die vollständige Konfiguration Ihrer Cloud in lesbarer Form. Kein Passwort, kein Token und kein Schlüssel kommt darin vor.'
    ,settingsRepoPreparing: 'Es wird ermittelt, was genau geschrieben wird …'

    ,installReadyHint: 'Bisher wurde nichts verändert. Den Plan unten zu genehmigen ist der erste Schritt, der das tut.'
    ,installWatching: 'Argo CD liest jetzt Ihr Einstellungs-Repository und installiert Ihre Auswahl in der richtigen Reihenfolge. Besonders Zertifikate können dauern.'
    ,installDone: 'Der Cluster meldet die erwarteten Bestandteile als bereit.'

    ,handoffNextHeading: 'Nächster Schritt'
    ,handoffActionCA: 'Zertifizierungsstelle des Clusters anlegen und auf diesem Gerät vertrauen'
    ,handoffActionEnroll: 'Diesen Computer ins private Netz aufnehmen'
    ,handoffActionOwner: 'Passkey als erste Eigentümerin bzw. erster Eigentümer registrieren'
    ,handoffComplete: 'Die Verwaltung ist privat. Die vorübergehenden Zugänge sind geschlossen.'

    ,finishOutstanding: 'Noch offen'
    ,finishHealthy: 'Alles, was diese Konsole prüfen kann, ist in Ordnung.'
    ,finishConsoleHint: 'Ab jetzt verwalten Sie diese Installation unter ihrer eigenen privaten Adresse über das private Netz. Der Launcher hat seine Aufgabe erfüllt und kann geschlossen werden.'
    ,finishProtectionMissing: 'Es ist noch nicht nachgewiesen, dass eine Kopie an einem anderen Ort ankommt. Bis dahin bedeutet ein Hardwaredefekt das Ende dieser Gemeinschaft.'

    ,activityEmpty: 'Für diese Installation wurde bisher nichts festgehalten.'
    ,activityRunStarted: 'Arbeit begonnen'
    ,activityRunCheckpoint: 'Fortschritt festgehalten'
    ,activityRunFailed: 'Arbeit fehlgeschlagen'
    ,activityOffsiteProposed: 'Änderung für die Auslagerung vorgeschlagen'
    ,activityDecommissionApproved: 'Stilllegung genehmigt'
    ,activityDecommissionCompleted: 'Stilllegung abgeschlossen'
    ,activityRedactionNote: 'Zugangsdaten erscheinen weder hier noch in Plänen oder Exporten.'

    ,manageDetails: 'Angaben zur Installation'
    ,manageCredentials: 'Was im Tresor liegt'
    ,manageDiagnostics: 'Diagnose'
    ,retireNotYet: 'Es gibt nichts stillzulegen: Diese Installation wurde noch nicht aufgebaut. Sie von diesem Computer zu entfernen genügt.'
