...
### Roundcube Debugging & Fix
**Status: Complete**
*   Implemented auto-login verification.
*   **Debugged and resolved IMAP authentication failures**:
    *   Roundcube attempts OAUTHBEARER authentication against Stalwart.
    *   Tokens generated by Keycloak include `aud: ["roundcube", "account"]` as an array.
    *   Stalwart's `requireAudience` check rejected the array structure.
    *   Removed `requireAudience` and mapped `claimUsername` to `email` inside `stalwart-init-job.yaml` so tokens are verified successfully via signature and users are accurately identified.
*   The tests verify reaching the inbox and the existence of the Compose button.
