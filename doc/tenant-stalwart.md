...
- **Cluster CIDRs allow-listed against fail2ban**: Stalwart's built-in fail2ban sees only proxy/NAT addresses, never real clients — all ingress HTTP arrives from Traefik's pod IP, and k3s ServiceLB SNATs external SMTP/IMAP to the flannel gateway `10.42.0.1`. On production's first day, internet port-scanners probing port 25 got `10.42.0.1` **permanently** banned, Alertmanager was banned for failed alert-mail deliveries, and Traefik's pod IP was banned too — after which *every* HTTP request (JMAP included) returned a bare `403 Forbidden` problem+json and Bulwark SSO failed at the callback step with no useful error anywhere (Stalwart logs nothing for banned-IP rejections). The init job now creates `AllowedIp` entries for `10.42.0.0/16`, `10.43.0.0/16`, and `127.0.0.0/8` (idempotent). Diagnosis shortcut: `stalwart-cli ... query BlockedIp --json` from the `mail-provisioner` pod, which has the CLI and admin credentials in env.
