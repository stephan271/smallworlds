...
- **Redirect URIs are now reconciled, not create-only**: the job originally exited as soon as `keycloak-secret` existed, so a base release that *changed* a tenant's `REDIRECT_URIS` never reached an already-provisioned client. This bit for real: Bulwark v1.5.0 moved its OIDC callback from `/api/auth/callback` to `/{locale}/auth/callback` (widened to `webmail.<domain>/*` in `855fa5e`), but production's client — registered under the older base tag — kept the old URI, and every SSO login failed with Keycloak's `Invalid parameter: redirect_uri` until the client was updated by hand. The job now re-asserts `redirectUris` on every sync. Deliberately **best-effort** on the already-provisioned path: if Keycloak isn't ready at that moment the reconciliation is skipped (never waited for), so tenant syncs can't hang or fail on a Keycloak that is down or mid-upgrade — the create path's blocking wait loop only applies to first-time provisioning, where the tenant can't start without the secret anyway.
