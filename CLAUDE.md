...
`argocd-apps.md` (sync waves) and `bases.md` (init job bases) are summarized above. `local-deployment.md` covers the LAN/local-server target (requirements, DNS/TLS differences, lifecycle). `pod-archive.md` covers the append-only personal data pods: the `pod-gateway` protocol, the Immich export bridge, and the member home device — read it before touching `tenants/pod-gateway` or `tenants/immich/pod-export`, and note that Immich originals are deliberately *not* covered by `pv-backup` or the offsite replicator any more (`docs/adr/0047`). `storage-and-backup.md` maps where every app's data lives (PVs/PVCs, databases, Garage buckets), the backup chain and its known gaps, and per-layer scale-up procedures — consult it before touching CNPG `backup:` blocks, PV sizes, or anything under `tenants/backup-replicator`/`velero`. `mail.md` covers who sends mail and how, and the design (`docs/adr/0049`, not yet implemented) under which mail is opt-in entirely — onboarding uses out-of-band action-token links and alerting moves to HTTP push, with a fixed `smtp-relay` endpoint only for clusters that do select mail — read it before touching any SMTP setting or `ADMIN_EMAIL`. `hardening.md` is the single reference for why member apps are public while operator interfaces are tailnet-restricted, and for the edge/auth/containment layers that pay for that — read it before changing an ingress annotation, a rate limit, or a NetworkPolicy. Also present: `bases.md`, `plane-architecture.md`, `tenant-dashboard.md`, `tenant-forgejo.md`, `tenant-hermes.md`, `tenant-immich.md`, `tenant-keycloak.md`, `tenant-nextcloud.md`, `tenant-operator-console.md`, `tenant-other.md`, `tenant-stalwart.md` — check the relevant one before making non-trivial changes to that subsystem, as several encode hard-won fixes (version incompatibilities, ordering bugs) that aren't obvious from the manifests alone.
